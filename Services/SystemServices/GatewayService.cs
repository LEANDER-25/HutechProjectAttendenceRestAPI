...
namespace RESTAPIRNSQLServer.Services.SystemServices
{
    public class GatewayService : IGatewayService
    {
        ...
        {
            bool isExist = await _context.Students.AnyAsync(s => s.Email.Equals(student.Email) || s.StudentCode == student.BusinessCode);
            if(isExist)
            ...
            var academicYearSID = await _context.AcademicYears.Where(y => y.AcademicYearName.Equals(student.AcademicYear)).Select(y => y.AyId).FirstOrDefaultAsync();
            newStudent.AcademicYear = academicYearSID;

            return true;
        }

        public Task<bool> TeacherRegister(TeacherRegisterDTO teacher)
        {
